...
int main() {
  ...
  *it = 1;
  ...
  // because the constructor that take a single number as its parameter is
  // explicite we can use explicit type cast to cast a number of type
  // vector<int>::size_type to a vector<int>
  vector<int> castv(
      static_cast<vector<int>>(static_cast<vector<int>::size_type>(10)));
  for (auto i : castv) {
    cout << i << ' ';
  }
  cout << endl;
  ...
  cout << arr[50] << ' ' << arr_default[50]
       << endl; // both of which are initialized to 0;  in another environment,
                // the later one is not initialized
  arr = arr_default; // not as same as built-in array, copy
  arr[2] = 10;
  arr = {0, 1}; // assignment, it should not be allowed but no warning or error
  // in fact, brace elision take care of it and finish the rest of the right
  // brace object
  cout
      << arr[0] << ' ' << arr[1] << ' ' << arr[2]
      << endl; // upper assignment change the whole arr, so arr[2] is value-init
