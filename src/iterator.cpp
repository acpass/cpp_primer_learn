...
int main() {
  ...
  partition_copy(s.cbegin(), s.cend(), os_iter, oss_iter,
                 [](char c) { return c == 'a' ? 1 : 0; });
  cout << endl;
  ...
  if (fd_in.is_open()) {
    ...
    while (inf_iter1 != eof && inf_iter2 != eof) {
      *os_iter = *inf_iter1++;
      *os_iter = *inf_iter2++;
      // dereference just return a element bound to the iterator (read
      // previously), it is the ++ operator who really read a element from the
      // stream except for the first read, which should be done when
      // initializing but can be delay to sync different iterator bound to the
      // same stream (lazy read)
    }
    cout << "$eof$";
    cout << endl;
  } else {
  ...
  cout << string(last_word.base(), s2.crbegin().base())
       << endl; // base() return the next iterator for a "normal" last_word
