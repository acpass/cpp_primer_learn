...
void lambda_test(int a, bool (*foo)(int)) { foo(a); }

void show(int first, int second) { cout << first << ' ' << second << endl; }

int main() {
  vector<int> v{1, 2, 3, 4, 5, 1, 5, 3, 6, 8, 4, 3};
  sort(v.begin(), v.end(), [](int a, int b) -> bool { return a < b; });
  auto iter = find_if(v.cbegin(), v.cend(), [](int a) { return a > 5; });
  ...
  auto foo2 = [] { cout << __FILE__ << ' ' << __LINE__ << endl; };
  ...
  for_each(v.cbegin(), v.cend(), [](const int a) {
    if (a > 3)
      cout << a << ' ';
  });
  cout << endl;
  ...
  auto outputfoo([](ostream &os, const string &s) { os << s; });
